...
main()
{
  ...
  # Status left
  if $show_powerline; then
    tmux set-option -g status-left "#[bg=${light_purple},fg=${dark_gray}]#{?client_prefix,#[bg=${orange}],} ${left_icon} #[fg=${light_purple},bg=${gray}]#{?client_prefix,#[fg=${orange}],}${left_sep}"
    powerbg=${gray}
  else
    tmux set-option -g status-left "#[bg=${light_purple},fg=${dark_gray}]#{?client_prefix,#[bg=${orange}],} ${left_icon}"
  fi
